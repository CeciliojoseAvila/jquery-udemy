(function () {
  ...
  $.smallBox = function (opciones) {
    ...
    html +=
      '<div id="' +
      sbID +
      '" class="sb-body" style="background-color:' +
      opciones.color +
      '">'; // Aqui debemos colocar el color
  ...
  function animar_salida(sbID) {
    ...
    tl.to($smallBox, 1, { x: "+= 200px" })
      .to(
        $smallBox,
        1,
        { opacity: 0, onComplete: remover_contenido, onCompleteParams: [sbID] },
        "-=1"
      )
      .to($smallBox, 0.8, {
        height: "0px",
        marginTop: "0px",
        onComplete: remover_notificacion,
  ...
  function animar_entrada(sbID) {
    ...
    var tl = new TimelineMax();
    tl.from($smallBox, 1, { x: "+= 100px", ease: Bounce.easeOut }).from(
      $smallBox,
      1,
      { opacity: 0 },
      "-=1"
